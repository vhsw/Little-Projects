...
class myWindow(arcade.Window):
    ...
    def on_draw(self):
        ...
        arcade.draw_text('Press arrow keys to push pendulums', -300, 285, arcade.color.BLACK, )

        G_CONST = 9.8 /100
        denom = (2 * p1.mass
                 + p2.mass
                 - p2.mass * math.cos(2*p1.angle - 2*p2.angle))
        accel_1 = (-G_CONST * (2*p1.mass + p2.mass) * math.sin(p1.angle)
                   ...
                   * (p2.velocity ** 2 * p2.length + p1.velocity ** 2 *
                       p1.length * math.cos(p1.angle - p2.angle))) / (denom * p1.length)
        accel_2 = (2 * math.sin(p1.angle - p2.angle)
                   * (p1.velocity**2 * p1.length * (p1.mass + p2.mass)
                      + G_CONST * (p1.mass + p2.mass) * math.cos(p1.angle)
                      + p2.velocity**2 * p2.length * p2.mass * math.cos(p1.angle - p2.angle))) / (denom * p2.length)
        p1.update(accel_1)
        p2.update(accel_2)
